photo ={
    ...
    init: function () {
        ...
        $.getJSON("/json/photos-life.json", function (data) {
            that.render(that.page, data);
            //that.scroll(data);
        });
        $.getJSON("/json/photos-ship.json", function (data) {
            ...
            li += '<div class="card" style="width:250px">' +
                    ...
                    //href 和 src 的链接地址是相册照片外部链接，也可以放博客目录里
                      '<a data-fancybox="gallery" href="'+ this.cosUrl +''+ imgNameWithPattern + '?raw=true" data-caption="' + imgName + '">' +
                        '<img srcset="'+ this.cosUrl + '' + imgNameWithPattern + '?raw=true" src="'+ this.cosUrl +' ' + imgNameWithPattern + '?raw=true"/>' +
                      '</a>' +
                    '</div>' +
            ...
            li += '<div class="card" style="width:250px">' +
                    ...
                    //href 和 src 的链接地址是相册照片外部链接，也可以放博客目录里
                      '<a data-fancybox="gallery" href="'+ this.cosUrl2 +''+ imgNameWithPattern + '?raw=true" data-caption="' + imgName + '">' +
                        '<img srcset="'+ this.cosUrl2 + '' + imgNameWithPattern + '?raw=true" src="'+ this.cosUrl +' ' + imgNameWithPattern + '?raw=true"/>' +
                      '</a>' +
                    '</div>' +
                    // '<div class="TextInCard">' + imgName + '</div>' +  //图片下显示文件名作为说明的功能
                  '</div>'
